 ...
 *
 *    eat_lcd_light_sw(EAT_FALSE);eat_kpled_sw(EAT_FALSE);
...
eat_bool eat_modem_data_parse(u8* buffer, u16 len, u8* param1, u8* param2)
{
    ...
    /*param:%d,extern_param:%d*/
     buf_ptr = (u8*)strstr((const char *)buffer,"param");//函数用于判断字符串param是否是buffer的子串。
                                                         //如果是，则该函数返回param在buffer中首次出现的地址；否则，返回NULL。
    if( buf_ptr != NULL)
    {
        sscanf((const char *)buf_ptr, "param:%d,extern_param:%d",(int*)param1, (int*)param2);//从字符串读取格式化输入
    ...
    if( 1 == param1 )
    {
        ...
        if( 1 == param2 )
        {
            eat_trace("GPIO test 1,1 start");
              //配置gpio的属性      输出拉低
            eat_gpio_setup(EAT_PIN3_UART1_RTS, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//3 
            eat_gpio_setup(EAT_PIN4_UART1_CTS, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//4
            eat_gpio_setup(EAT_PIN5_UART1_DCD, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//5
            eat_gpio_setup(EAT_PIN6_UART1_DTR, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//6
            eat_gpio_setup(EAT_PIN7_UART1_RI, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//7
            eat_gpio_setup(EAT_PIN41_NETLIGHT, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//41
            eat_gpio_setup(EAT_PIN42_STATUS, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//42
            eat_gpio_setup(EAT_PIN23_UART2_RXD, EAT_GPIO_DIR_OUTPUT, EAT_GPIO_LEVEL_LOW);//23
    ...
    }else if(param1 == 2)
     {
         if( 1== param2 )
          {
              eat_lcd_light_sw(EAT_TRUE, EAT_BL_STEP_20_MA );//LCD 背光点亮
              eat_kpled_sw(EAT_TRUE);//点亮键盘背光
          }else if( 2 == param2 )
          {
              eat_lcd_light_sw(EAT_FALSE, 0);//LCD 背光灭
              eat_kpled_sw(EAT_FALSE);//灭键盘背光
          }
    }else if(param1 == 3)
    {
        if( 1== param2 )
...
void app_main(void *data)
{
    ...
    eat_modem_write("AT+CEAT=1,1\r\n",16);//发送数据至MODEM侧
    while(EAT_TRUE)
    {
        eat_get_event(&event);//获取队列消息
        eat_trace("MSG id%x", event.event);
        switch(event.event)
        {
            case EAT_EVENT_MDM_READY_RD://EAT 接收到Modem 发过来的数据
                {
                    u8 param1,param2;
                    len = 0;
                    len = eat_modem_read(buf, 2048);//接收数据
                    if(len > 0)
                    {
                        //获取测试参数
